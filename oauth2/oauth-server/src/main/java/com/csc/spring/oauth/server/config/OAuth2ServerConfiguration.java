...
import com.csc.spring.oauth.server.domain.oauth.CustomJdbcClientDetailsService;
import com.csc.spring.oauth.server.service.OauthService;
import com.csc.spring.oauth.server.service.UserService;
import com.csc.spring.oauth.server.web.oauth.OauthUserApprovalHandler;
    ...
    protected static class UnityResourceServerConfiguration extends ResourceServerConfigurerAdapter {
        ...
        public void configure(HttpSecurity http) throws Exception {
            http
                    ...
                    .requestMatchers().antMatchers("/spring-oauth-server/unity/**")
                    .and()
                    .authorizeRequests()
                    .antMatchers("/spring-oauth-server/unity/**").access("#oauth2.hasScope('read') and hasRole('UNITY')");
    ...
    protected static class MobileResourceServerConfiguration extends ResourceServerConfigurerAdapter {
        ...
        public void configure(HttpSecurity http) throws Exception {
            http
                    ...
                    .and()
                    // 所有以 /m/  开头的 URL属于此资源
                    .requestMatchers().antMatchers("/spring-oauth-server/m/**")
                    .and()
                    .authorizeRequests()
                    .antMatchers("/spring-oauth-server/m/**").access("#oauth2.hasScope('read') and hasRole('MOBILE')");
